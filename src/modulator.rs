...
impl Adsr {
    ...
    pub fn update_notes_held_count(&mut self, notes_pressed: i32, notes_released: i32) {
        ...
        println!("AdsrGain: Updated notes held count to: {}", self.notes_held_count);
        ...
        loop {
            match self.stage {
                ...
                AdsrStages::HeldAttack => {
                    return self.gain_at_stage_start + self.relative_gain_at_stage_end * (
        ...
        match self.stage {
            AdsrStages::HeldAttack => {
                if self.phase_time >= self.params.attack_duration {
                    ...
                    self.relative_gain_at_stage_end = self.params.sustain_level - self.gain_at_stage_start;
                    self.phase_time -= self.params.attack_duration;
                }
            },
            AdsrStages::HeldDecay => {
